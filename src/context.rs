...
impl Context {
    ...
    pub fn select_font_face(&self, family: &str, slant: FontSlant, weight: FontWeight){
        unsafe {
            ffi::cairo_select_font_face(self.get_ptr(), family.lend_to_glib().0, slant, weight)
        }
    }
    ...
    pub fn show_text(&self, text: &str){
        unsafe {
            ffi::cairo_show_text(self.get_ptr(), text.lend_to_glib().0)
        }
    }
    ...
    pub fn text_extents(&self, text: &str) -> TextExtents {
        ...
        unsafe {
            ffi::cairo_text_extents(self.get_ptr(), text.lend_to_glib().0, &mut extents);
        }
        extents
    }
    ...
    pub fn text_path(&self, str_: &str){
        unsafe {
            ffi::cairo_text_path(self.get_ptr(), str_.lend_to_glib().0)
        }
    }
